-- Tạo Database
CREATE DATABASE QuanLyBanHang;
GO
USE QuanLyBanHang;
GO

-- Bảng Loại Mặt Hàng
CREATE TABLE LoaiMatHang (
    MaLoai INT PRIMARY KEY IDENTITY(1,1),
    TenLoai NVARCHAR(100)
);

-- Bảng Mặt Hàng
CREATE TABLE MatHang (
    MaHang INT PRIMARY KEY IDENTITY(1,1),
    TenHang NVARCHAR(100),
    DonGia FLOAT,
    MaLoai INT FOREIGN KEY REFERENCES LoaiMatHang(MaLoai)
);

-- Bảng Khách Hàng
CREATE TABLE KhachHang (
    MaKH INT PRIMARY KEY IDENTITY(1,1),
    TenKH NVARCHAR(100),
    DienThoai NVARCHAR(20)
);

-- Bảng Đơn Hàng
CREATE TABLE DonHang (
    MaDH INT PRIMARY KEY IDENTITY(1,1),
    NgayLap DATE,
    MaKH INT FOREIGN KEY REFERENCES KhachHang(MaKH)
);

-- Bảng Chi Tiết Đơn Hàng
CREATE TABLE ChiTietDonHang (
    MaDH INT,
    MaHang INT,
    SoLuong INT,
    FOREIGN KEY (MaDH) REFERENCES DonHang(MaDH),
    FOREIGN KEY (MaHang) REFERENCES MatHang(MaHang),
    PRIMARY KEY (MaDH, MaHang)
);


-- Loại mặt hàng
INSERT INTO LoaiMatHang (TenLoai) VALUES (N'Đồ uống'), (N'Đồ ăn'), (N'Đồ gia dụng');

-- Mặt hàng
INSERT INTO MatHang (TenHang, DonGia, MaLoai) VALUES 
(N'Cà phê', 20000, 1),
(N'Trà sữa', 25000, 1),
(N'Bánh mì', 10000, 2),
(N'Nồi cơm điện', 2500000, 3);

-- Khách hàng
INSERT INTO KhachHang (TenKH, DienThoai) VALUES 
(N'Nguyễn Văn Đạt', '0123456789'),
(N'Đoàn Xuân Nhi', '0987654321'),
(N'Bùi Khắc Huy', '0837607568');

-- Đơn hàng + Chi tiết đơn hàng
INSERT INTO DonHang (NgayLap, MaKH) VALUES 
('2023-12-11', 2),
('2024-04-01', 1), 
('2024-04-15', 2),
('2024-04-20', 1),
('2024-10-27', 2),
('2024-12-31', 3);

INSERT INTO ChiTietDonHang VALUES 
(6, 4, 3),
(1, 1, 2),
(1, 2, 3),
(2, 4, 1),
(3, 3, 5),
(3, 1, 1),
(3, 2, 1),
(4, 2, 5),
(5, 4, 2),
(5, 3, 5);

SELECT 
    dh.NgayLap,
    kh.TenKH,
    mh.TenHang,
    ct.SoLuong,
    mh.DonGia,
    (ct.SoLuong * mh.DonGia) AS ThanhTien
FROM DonHang dh
JOIN KhachHang kh ON dh.MaKH = kh.MaKH
JOIN ChiTietDonHang ct ON dh.MaDH = ct.MaDH
JOIN MatHang mh ON ct.MaHang = mh.MaHang
ORDER BY dh.NgayLap, kh.TenKH;
 

 SELECT kh.TenKH as 'Tên khách hàng', 
 mh.TenHang as 'Tên mặt hàng', 
 ct.SoLuong as 'Số lượng', 
 mh.DonGia as 'Đơn giá', 
 (ct.SoLuong * mh.DonGia) AS ThanhTien 
 FROM DonHang dh 
 JOIN KhachHang kh 
 ON dh.MaKH = kh.MaKH 
 JOIN ChiTietDonHang ct 
 ON dh.MaDH = ct.MaDH 
 JOIN MatHang mh 
 ON ct.MaHang = mh.MaHang
 WHERE dh.NgayLap = '2023-12-11' ORDER BY kh.TenKH;